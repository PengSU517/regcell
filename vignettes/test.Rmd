...
```{r}
fit0 = sregcell_lambda(y = y, x = x, lambda_zeta = 1, lambda_delta = 2.56, lambda = 1,
                               softbeta = FALSE, softdelta = TRUE, softzeta = TRUE,
                               alpha = 0.5)
fit0$betahat
...
library(mfp)
data("bodyfat")
y = robustHD::robStandardize(bodyfat$brozek)
# y = bodyfat$brozek
x = robustHD::robStandardize(bodyfat[,-c(1:3)])
x = as.matrix(x)
...
# fit0$betahat
# 
# fit00 = regcell::Rlars(yscl,x)
# sigmahat00 = fit00$sigmahat # tend to overestimate
# sigmahat00
# fit00$betahat
```
...
```{r fig.width=6, fig.height=6}
system.time({fit = sregcell_lambda(y = yscl, x = x, 
                               lambda_delta = 20, lambda_zeta = 1, lambda = 0,
                               softbeta = FALSE, softdelta = TRUE, softzeta = TRUE,
                               alpha = 0.5)})
...
plot((x - delta)%*%betahat, yscl - zeta)
plot(x%*%betahat+fit$intercept,yscl)
plot((x - delta)[,1],(yscl - zeta))


plot(x%*%betahat,yscl, pch = 19, cex = 0.7)
points((x - delta)%*%betahat, yscl - zeta, pch = 1, col = "red")

plot(x[,3],yscl, pch = 19, cex = 0.7)
points((x - delta)[,3],(yscl - zeta), col = "red")

# 当lambda_delta = 10, lambda_z = 2, 下方的outlier没有shrinkage
# 好像是截距项的估计很不正常
