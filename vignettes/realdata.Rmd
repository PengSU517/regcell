...
featuredat = readr::read_tsv("E-MEXP-1618.sdrf.txt") |> 
  ...
  dplyr::select(id,
                hip_t = characteristics_total_hip_t_score) |> arrange(id) ####sort obs by patient ID
...
```{r fig-outliervis, fig.height=7, fig.width=10}
...
On average, the screened genes exhibit a contamination rate of $3.61\%$, with the probe $\textrm{236831\_at}$ having the highest contamination rate of $9.52\%$. 
Among the observations, the thirteenth observation with the patient ID 20 shows the highest contamination rate at $22\%$.
